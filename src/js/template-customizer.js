...
class TemplateCustomizer {
  ...
  _setup(_container = document) {
    ...
    const resetBtnCb = () => {
      ...
      // For demo purpose only (Django)
      document.cookie = 'django_language=en;path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;'
      document.cookie = 'django_text_direction=ltr;path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;'
      ...
      if (!this._hasControls('rtl') || !rtlSupport) {
        ...
        const rtlCb = e => {
          ...
          if (e.target.value === 'rtl') {
            this._setSetting('Lang', 'ar')
            document.cookie = 'django_language=ar;path=/'
            document.cookie = 'django_text_direction=rtl;path=/'
          } else {
            this._setSetting('Lang', 'en')
            document.cookie = 'django_language=en;path=/'
            document.cookie = 'django_text_direction=ltr;path=/'
          }
